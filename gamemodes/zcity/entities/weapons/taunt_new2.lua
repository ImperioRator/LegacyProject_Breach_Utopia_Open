...
SWEP.PrintName = "Веселый танец 2" 
...
function SWEP:Think()
	if SERVER then
		if nextheal <= CurTime() and self.Owner.ForceAnimSequence == self.Owner:LookupSequence("wos_fn_rambunctious") then
...
function SWEP:PrimaryAttack( right )
	...
	if SERVER then
		...
		if self.Owner.ForceAnimSequence == self.Owner:LookupSequence("wos_fn_rambunctious") then
		...
		elseif self.Owner.ForceAnimSequence == nil and self.Owner:GetMoveType() == MOVETYPE_WALK then
			...
			self.Owner:SetForcedAnimation('wos_fn_rambunctious', 60, function()
