if SERVER then
	...
	net.Receive( "ScreengrabRequest", function( _, ply )
		...
		if ply:CanScreengrab() then
			ply:rtxappend( sg.green, "Инициализация" )
	...
	net.Receive( "ScreengrabSendPart", function( _, ply )
		...
		if not ply.data then
			ply.data = {}
			ply.data[ 1 ] = data
			sendto:rtxappend( sg.blue, "Сбор первой части" )
		else
			local num = table.getn( ply.data ) + 1
			ply.data[ num ] = data
			sendto:rtxappend( sg.blue, "Получена " .. num .. STNDRD( num ) .. " часть" )
		end
		if table.getn( ply.data ) == ply.parts then
			ply.IsSending = nil
			sendto:rtxappend( sg.green, "Подготовка данных к отправке [" .. ply.parts .. " parts]" )
			local i = 1
			timer.Create( "SendDataBack", 0.1, ply.parts, function()
				...
				net.Send( sendto )
				sendto:rtxappend( sg.yellow, "Отправка " .. i .. STNDRD( i ) .. " части" )
				i = i + 1
			end )
		end
	end )
	net.Receive( "ScreengrabFinished", function( _, ply )
		...
		ply.isgrabbing = nil
		_ply.isgrabbing = nil
		ply:rtxappend( sg.green, "Завершено" )
	end )
	...
	hook.Add( "PlayerDisconnected", "ScreengrabInterrupt", function( ply )
		if ply.IsSending then
			...
			ply.isgrabbing = nil
			_ply.isgrabbing = nil
			_ply:rtxappend( sg.red, "Целевой игрок отключился до завершения передачи данных" )
...
if CLIENT then
    ...
    net.Receive( "StartScreengrab", function()
        ...
        sgQuality = net.ReadUInt(32)
        sgPly = net.ReadEntity()
    end)
    ...
    hook.Add("PostRender", "Screengrab", function()
        ...
        cl_rtxappend2( sg.green, "Инициализация", sgPly )        
        local function capture( q )
            ...
            cl_rtxappend2( color_white, "Захват " .. len .. " байт", sgPly )
            local parts = math.ceil( len / split )
            cl_rtxappend2( color_white, parts .. " частей", sgPly )
            local partstab = {}
            for i = 1, parts do
                local min
                local max
            ...
            net.Start( "ScreengrabInitCallback" )
                net.WriteEntity( sgPly )
                net.WriteUInt( amt, 32 )
                net.WriteUInt( len, 32 )
                net.WriteFloat( CurTime(), 32 )
            net.SendToServer()
            cl_rtxappend2( Color( 0, 255, 0 ), "Подготовка данных к отправке", sgPly )
            local i = 1
            timer.Create( "ScreengrabSendParts", 0.1, amt, function()
                ...
                net.SendToServer()
                cl_rtxappend2( Color( 255, 255, 0 ), "Отправка " .. i .. STNDRD( i ) .. " части", sgPly )
				net.Start( "Progress" )
	...
	local function DisplayData( str, name )
		...
		if not name then
			main:SetTitle( "Скриншот от " .. LocalPlayer().gfname .. " (" .. string.len( str ) .. " байт, заняло " .. elapsedtime .. " секунд)" )
		else
	...
	net.Receive( "ScreengrabInterrupted", function()
		cl_rtxappend( sg.red, "Соединение с целевым игроком прервано" )
	...
	net.Receive( "SendPartBack", function()
		...
		if table.getn( LocalPlayer().sgtable ) == LocalPlayer().parts then
			...
			cl_rtxappend( sg.green, "Завершено" )
	...
	function OpenSGMenu()
		...
		main.Paint = function()
			...
			surface.DrawText( "Меню скриншотов" )
		end
		...
		plys:SetPos( 5, 5 )
		plys:SetSize( 150, 25 )
		plys:AddChoice( "Выберите игрока", nil, true )
		plys.curChoice = "Выберите игрока"
		...
		execute:SetText( "Сделать скриншот" )
		...
		auto:SetPos( 5, 83 )
		auto:SetText( "Автоматически открывать" )
		auto:SetDark( true )
		auto:SizeToContents()
		...
		files.OnRowRightClick = function( main, line )
			local menu = DermaMenu()
				...
				menu:AddOption( "Просмотреть скриншот", function()
		...
		save:SetPos( 4, 220 )
		save:SetSize( 150, 20 )
		save:SetText( "Сохранить изображение" )
		...
		save.DoClick = function()
			...
			cl_rtxappend( Color( 255, 100, 0 ), "Сохранение в файл: " .. name .. " (" .. string.len( text ) .. " байт)" )
			file.Write( "screengrabs/" .. name, text )
			timer.Simple( 1, function()
				if file.Exists( "screengrabs/" .. name, "DATA" ) then
					cl_rtxappend( Color( 255, 100, 0 ), "Скриншот сохранен!" )
				else
					cl_rtxappend( Color( 255, 0, 0 ), "Ошибка: Скриншот не сохранен!" )
				end
			end )
		end
