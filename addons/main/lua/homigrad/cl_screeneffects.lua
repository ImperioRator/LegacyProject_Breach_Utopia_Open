...
hook.Add("Post Post Processing", "ItHurts", function()
	...
	if lply.GTeam and lply:GTeam() == TEAM_SPEC and not isRecentlyDead then stopthings() return end
	if IsValid(PainStation) then
		PainStation:SetVolume(0)
	end
	
	if !lply:Alive() and !IsValid(spect) and not isRecentlyDead then stopthings() return end
	if !lply:Alive() and viewmode != 1 and not isRecentlyDead then stopthings() return end
	local organism = (lply:Alive() or isRecentlyDead) and lply.organism or (IsValid(spect) and spect.organism)
	...
	local shock = ((org.shock or 0) * 1 + (1 - org.consciousness) * 40) * fade
	shockLerp = LerpFT(0.01, shockLerp or 0, shock)
	
	local consciousness = 1 - ((1 - org.consciousness) * fade)
	consciousnessLerp = LerpFT(consciousness < (consciousnessLerp or 1) and 1 or 0.01, consciousnessLerp or 1, consciousness)
	
	PainLerp = LerpFT(0.05, PainLerp, math.max(pain * (org.otrub and 0.2 or 1), 0))
	assimilatedLerp = LerpFT(0.01, assimilatedLerp, (org.assimilated or 0) * fade)
	tempLerp = LerpFT(0.01, tempLerp, 36.6)
	...
	if (PainLerp > 0.001 or shockLerp > 5) or org.otrub then
		...
		-- Меньше интенсивности и меньше виньетки
		vignetteMat:SetFloat("$c0_z", org.otrub and 2 or ((pain / 40 + math.max(shock - 5, 0) / 3) * 0.3)) //ColorIntensity
		vignetteMat:SetFloat("$c1_y", org.otrub and 4 or ((pain / 40 + math.max(shock - 5, 0) / 3) * 0.3)) //Vignette
		...
		-- Меньше размытия (Lerp) и красной пелены
		painMat:SetFloat("$c1_x", math.Clamp(pain / 90, 0, 0.75) * 0.3) //Lerp
		painMat:SetFloat("$c1_y", math.Clamp(pain / 90, 0, 0.75) * 0.3) //Vignette
