...
local bones = hg.TPIKBonesLH
	for _, bone in ipairs(bones) do
		...
		local is_hand = (bone == "ValveBiped.Bip01_L_Hand" or bone == "ValveBiped.Bip01_R_Hand")
		local target_pos = wm_bonematrix:GetTranslation()
		
		if is_hand then
			target_pos.x = math.Clamp(target_pos.x, pos.x - 38, pos.x + 38)
			target_pos.y = math.Clamp(target_pos.y, pos.y - 38, pos.y + 38)
			target_pos.z = math.Clamp(target_pos.z, pos.z - 38, pos.z + 38)
		else
			-- ОЧЕНЬ ВАЖНО: Для пальцев сохраняем родную позицию из модели игрока, 
			-- перенося только вращение. Это навсегда избавляет от эффекта "сосисок".
			target_pos = ply_bonematrix:GetTranslation()
		end
		...
		local m1 = Matrix()
		m1:SetAngles(wm_bonematrix:GetAngles())
		local m2 = Matrix()
		...
		ply_bonematrix:SetTranslation(is_hand and LerpVector(lerp, target_pos, ply_bonematrix:GetTranslation()) or target_pos)
		ply_bonematrix:SetAngles(q3:Angle())

		-- Применяем с наследованием, чтобы дети (и игнорируемые кости типа FingerX0) двигались правильно
