...
--\\ Remove cl models on cleanup
	...
	function ClientsideModel( model, ... )
		...
		local ent = hg.oldClientsideModel( model, ... )
		if IsValid( ent ) then
			table.insert( hg.ClientsideModels, ent )
		end
		return ent
	end
...
--\\ Supression
	if CLIENT then
		...
		hook.Add("PostEntityFireBullets","bulletsuppression2",function(ent,bullet)
			...
			if not IsValid(lply) or not lply:IsPlayer() then return end
			if !lply:Alive() or !lply.organism or lply.organism.otrub then return end
			if lply.GTeam and lply:GTeam() == TEAM_SPEC then return end
			if lply.GTeam and lply:GTeam() == TEAM_AR then return end
...
--\\ Can see or not
	...
	local next_vis_check = 0
	hook.Add("Think", "CanBeSeenOrNot", function()
		if CurTime() < next_vis_check then return end
		next_vis_check = CurTime() + 0.25 -- ОПТИМИЗАЦИЯ: Проверяем видимость 4 раза в секунду, а не 144!
		...
		for i = 1, #entities do
			...
			-- Упрощенная и сверхбыстрая проверка видимости (Dot Product)
			local vPos = v:WorldSpaceCenter()
			local dir = (vPos - origin)
			dir:Normalize()

			if forward:Dot(dir) < 0.3 then
...
--\\ move it to CL util
		...
		local function mouthmove(ply)
			...
			local weight = (ply:IsSpeaking() and math.Clamp( ply:VoiceVolume() * 10, 0, 2 )) or 0
