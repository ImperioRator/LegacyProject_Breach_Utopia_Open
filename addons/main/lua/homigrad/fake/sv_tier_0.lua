...
local 🎃 = 🔇.Run
🐔("OnEntityCreated", "bull_add", function(npc)
...
local IdealMassPlayer = 🎂.IdealMassPlayer
...
function 🎂.Ragdoll_Create(ply)
	...
	ragdoll:CallOnRemove("removeBull", function()
		🎂.queue_ragdolls[ragdoll] = nil

		if IsValid(ragdoll.bull) then
			ragdoll.bull:Remove()
		end
	end)
	ragdoll:AddCallback("PhysicsCollide", function(outEnt, data) 🎃("Ragdoll Collide", ragdoll, data) end)
	...
	for physNum = 0, ragdoll:GetPhysicsObjectCount() - 1 do
		...
		--print(ragdoll:GetBoneName(ragdoll:TranslatePhysBoneToBone(🎂.cachedmodels[model][ragdoll:GetBoneName(bone)])),ragdoll:GetBoneName(bone),IdealMassPlayer[ragdoll:GetBoneName(bone)])
		...
		local hitgroup = 🎂.bonetohitgroup[bonename]--( ent:IsPlayer() and tr.HitGroup or 🎂.bonetohitgroup[bonename])
		
		if 🎂.amputeetable[bonename] and ply.organism[🎂.amputeetable[bonename].."amputated"] then
		...
		if ply:InVehicle() then
			...
			//local pos, ang = 🎂.RotateAroundPoint2(pos, ang, vector_origin, vector_origin, Angle(-90,0,0))
			phys:SetPos(pos)
			phys:SetAngles(ang)
			...
			if fixbones[ragdoll:GetBoneName(bone)] then
				...
				weld:CallOnRemove("removeOwO", function()
					...
					//🔇.Run("CanExitVehicle", ply, veh)
					if !🎂.leaveveh then 🎂.fallfromveh = true end
					🎂.leaveveh = true
					ply:ExitVehicle()
...
local Ragdoll_Create = 🎂.Ragdoll_Create
...
🎂.ragdollFake = 🎂.ragdollFake or {}
--local ragdollFake = 🎂.ragdollFake
🐔("DoPlayerDeath", "Fake", function(ply)
	-- Если игрок валялся без сознания в виде рэгдолла ZCity, удаляем его
	if IsValid(ply.FakeRagdoll) then
		ply.FakeRagdoll:Remove()
		ply.FakeRagdoll = nil
	end

	-- Не создаем никаких новых рэгдоллов или статуй!
	-- Просто очищаем переменные, чтобы Breach мог заспавнить свой труп.
	ply:SetNWEntity("RagdollDeath", NULL)
	ply.RagdollDeath = nil
end)

🐔("PostPlayerDeath", "Garbage", function(ply)
...
🎂.queue_ragdolls = 🎂.queue_ragdolls or {}
🎂.humans_cached = 🎂.humans_cached or {}

🐔("SetupPlayerVisibility", "fuckragdolls", function( ply )
	local queue = 🎂.queue_ragdolls

	for ent, tbl in pairs(queue) do--ему, наверное, больно
		if not IsValid(ent) then queue[ent] = nil continue end
		if queue[ent].count == #🎂.humans_cached then queue[ent] = nil continue end
...
🐔("SetupPlayerVisibility", "ragdollview", function( ply )
	local ent = IsValid(🎂.ragdollFake[ply]) and 🎂.ragdollFake[ply] or ply:GetNWEntity("FakeRagdoll")
...
function 🎂.Fake(ply, huyragdoll, no_freemove, force)
	...
	//if ragdoll:GetVelocity():LengthSqr() < (200 * 200) or ply:InVehicle() then 🎂.SetFreemove(ply,not no_freemove) end

	🎂.ragdollFake[ply] = ragdoll
	ply.ActiveWeapon = ply:GetActiveWeapon()
	🎃("Fake", ply, ragdoll, listArmor)
...
🐔("PreCleanupMap","VSEM_VSTAT",function()
	for i, ply in player.Iterator() do
		🎂.FakeUp(ply)
	end
end)

🐔("PlayerSpawn", "!!!!!!", function() if OverrideSpawn then return false end end)
🐔("PlayerSpawn", "z", function() if OverrideSpawn then return false end end)
🐔("Player Spawn", "!!!!!!", function() if OverrideSpawn then return false end end)
🐔("Player Spawn", "z", function() if OverrideSpawn then return false end end)

util.AddNetworkString("Override Spawn")
function 🎂.OverrideSpawn(ply)
...
🐔("Should Fake Up","speedhuy",function(ply)
	if IsValid(ply.FakeRagdoll) then
...
🐔("Player Spawn", "fuckingremoveragdoll", function(ply)
...
🐔("CanControlFake","stunnednocontrol",function(ply,rag)
	if (ply.organism.stun - CurTime()) > 0 then return false end
end)

local util_TraceLine = util.TraceLine
function 🎂.FakeUp(ply, forced, instant)
	...
	if not forced and (not IsValid(ply.FakeRagdoll) or not ply:Alive() or 🎃("Should Fake Up", ply) ~= nil) then return false end
	...
	local pos = 🎂.GetUpPos(ply, posit, 50, 50)
		...
		if not instant then
			timer.Create("faking_up"..ply:EntIndex(), 1, 1, function()
				if IsValid(ragdoll) then
					...
					//pos = 🎂.GetUpPos(ply, posit, 50, 50) or oldpos
				end
...
function 🎂.GetCurrentCharacter(ply)
...
🐔("PlayerDisconnected", "Fake", function(ply) 🎂.ragdollFake[ply] = nil end)
🐔("PlayerFootstep", "CustomFootstep", function(ply) if IsValid(ply.FakeRagdoll) then return true end end)
function 🎂.RagdollOwner(ragdoll)
	if not IsValid(ragdoll) then return end
	local ply = ragdoll.ply
	return IsValid(ply) and ply.FakeRagdoll == ragdoll and ply
end

🐔("PlayerDisconnected", "🎂-killniers", function(ply)
	if ply:Alive() then
		local ragdoll = ply:GetNWEntity("RagdollDeath")
		if IsValid(ragdoll) then
			local newOrg = 🎂.organism.Add(ragdoll)
			table.Merge(newOrg,ply.organism)
			newOrg.alive = false
			newOrg.owner = ragdoll
			ragdoll:CallOnRemove("organism", 🎂.organism.Remove, ragdoll)
		end
		🎂.organism.Clear(ply.organism)
		if not ply:GetPos():WithinAABox(Vector(-3847, 3237, 211), Vector(-3216, 2354, -100)) then
			ply:Kill()
...
🐔("CanPlayerEnterVehicle","fake_enterveh",function(ply, veh)
	if veh.rags then
...
🐔("HG_OnWakeOtrub", "enterveh", function(ply)
...
🐔("HG_OnOtrub", "leaveveh", function(ply)
	if ply:InVehicle() then
		local veh = ply:GetVehicle()
...
🐔("PlayerLeaveVehicle","allowweapons",function(ply,veh)
	...
	if (!fast or ply.switchingseat) and ply:Alive() then
		🎂.FakeUp(ply, true, ply.switchingseat)
	else
		if ragdoll then
			ply:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
			--ply:SetSolidFlags(bit.bor(ply:GetSolidFlags(), FSOLID_NOT_SOLID, FSOLID_TRIGGER, FSOLID_USE_TRIGGER_BOUNDS))
...
🎂.ExitVehicle = 🎂.ExitVehicle or PLAYER.ExitVehicle

function PLAYER:ExitVehicle()
	//if 🎂.leaveveh then 🎂.ExitVehicle(self) 🎂.leaveveh = nil end
	🎂.ExitVehicle(self)
end
*/

🐔("CanExitVehicle","huyhuy",function(ply, veh)
	//return false
end)

local poses = {}

function 🎂.GetUpPos(target,pos,tries,starttries)
	...
	return 🎂.GetUpPos(target,pos,tries,starttries)
end
/*
local ent = Entity(1)
local ragdoll = ents.Create("prop_ragdoll")
...
🐔("Move","PushAwayRagdolls",function(ply, mv)
	do return end
	if not ply:Alive() or not 🎂.GetCurrentCharacter(ply):IsPlayer() then return end
...
🐔("Ragdoll Collide", "FallSounds", function(rag, data)
