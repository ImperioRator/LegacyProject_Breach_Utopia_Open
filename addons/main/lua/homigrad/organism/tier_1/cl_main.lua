hook.Add("PrePlayerDraw", "CanPlayerSeePlayer", function(ply, flags)
	...
	if ply:GetNWBool("CloakSAM", false) or hook.Run( "CanPlayerSeePlayer", lp, ply ) == false then
		return true
	end
	--print(ply)
	if ply:GetModel() == "models/1000shells/scp966/scp_966.mdl" then 
		if LocalPlayer().NVG == true then
			return false
		else
			return true
		end
...
local function MegaDSP(ply)
		...
		--print(avgDist)
		local targetDSP = 0
		if avgDist > 50000000 then targetDSP = 0
		elseif avgDist > 5000000 then targetDSP = 105
		elseif avgDist > 500000 then targetDSP = 3
		elseif avgDist > 50000 then targetDSP = 2
		elseif avgDist > 5000 then targetDSP = 104
		elseif avgDist <= 5000 then targetDSP = 102 end

		if ply.lastDSP ~= targetDSP then
			ply.lastDSP = targetDSP
			RunConsoleCommand("dsp_player", targetDSP)
...
hook.Add("RenderScreenspaceEffects", "organism-effects", function()
	local isRecentlyDead = lply.death_time_hg and (CurTime() - lply.death_time_hg) < 7
	local fade = isRecentlyDead and math.Clamp(1 - (CurTime() - lply.death_time_hg) / 7, 0, 1) or 1
	if lply.GTeam and lply:GTeam() == TEAM_SPEC and not isRecentlyDead then return end
	local spect = IsValid(lply:GetNWEntity("spect")) and lply:GetNWEntity("spect")
	local organism = (lply:Alive() or isRecentlyDead) and lply.organism or (viewmode == 1 and IsValid(spect) and spect.organism) or {}
	local new_organism = (lply:Alive() or isRecentlyDead) and lply.new_organism or (viewmode == 1 and IsValid(spect) and spect.new_organism) or {}

	if organism.owner == LocalPlayer() then
		if new_organism.otrub and !old then hook.Run("HG_OnOtrub", new_organism.owner) end
		old = new_organism.otrub
	end

	if not organism then return end
	local alive = lply:Alive() or (spect and spect:Alive()) or isRecentlyDead
	local health = ((lply:Alive() or isRecentlyDead) and lply:Health()) or 100

	local org = organism
	if not org.brain then return end
	
	-- Умножаем на fade, чтобы эффекты плавно исчезали
	local adrenaline = (org.adrenaline or 0) * fade
	local pulse = org.pulse or 70
	local pain = (org.pain or 0) * fade
	local blood = 5000 - ((5000 - (org.blood or 5000)) * fade)
	local bleed = org.bleed or 0
	local o2 = 30 - ((30 - (org.o2 and org.o2[1] or 30)) * fade)
	local brain = (org.brain or 0) * fade
	local otrub = (lply:Alive() or isRecentlyDead) and org.otrub or false
	local analgesia = (organism.analgesia or 0) * fade
	local disorientation = (org.disorientation or 0) * fade
	...
	-- Смягчаем резкость (Sharpen) в два раза
	DrawSharpen(k1 * 0.5, k1 * 0.25)
	...
	if (disorientationLerp > 1) and lply:Alive() or brain > 0 then
		...
		-- Смягчаем размытие при движении (Motion Blur)
		if not brain_motionblur then DrawMotionBlur(0.15 - math.Clamp(add2 / 1, 0, 0.1), add2 * 0.5, 0.001) end
		if disorientationLerp > 2 then
			local add = (disorientationLerp - 2) * 2
			local time = CurTime() * 3
			-- Убираем сильную тряску экрана (0.2 -> 0.05)
			local mul = math.Clamp(add / 16, 0, 0.05)
...
hook.Add("Player-Ragdoll think", "organism-think-client-blood", function(ply, ent, time)
	...
	local isRecentlyDead = ply.death_time_hg and (CurTime() - ply.death_time_hg) < 7
	if ply:IsPlayer() and ply.GTeam and ply:GTeam() == TEAM_SPEC and not isRecentlyDead then return end
