...
local function Trace_Bullet(box, hit, ricochet, org, organs, dmg, dmgInfo, dir)
	...
	local boneName = box[6] 
    local hitGroup = hg.bonetohitgroup[boneName] or HITGROUP_GENERIC

	local scaleMul = 1
    if ply.ScaleDamage and ply.ScaleDamage[hitGroup] then
        scaleMul = ply.ScaleDamage[hitGroup]
    end

	dmg = dmg * scaleMul
	--local old_i_dmg = dmg
	--dmg = 0
	--if name == "skull" or name == "brain" then
	--	print(old_i_dmg)
	--	print(org.owner.HeadResist)
	--	print(org.owner.BodyResist)
	--	PrintTable(org.owner.ScaleDamage)
	print(GetRoleTableSH(org.owner:GetRoleName())["headgear"])
	if name == "skull" or name == "brain" then
		if org.owner:GetRoleName():find("Juggernaut") then
			dmg = dmg / 32
			local ply = org.owner
			local hitpos = (hit and isvector(hit)) and hit or dmgInfo:GetDamagePosition()
			local effectdata = EffectData()
			effectdata:SetOrigin(hitpos)
			effectdata:SetNormal((dir or dmgInfo:GetDamageForce()):GetNormalized())
			effectdata:SetMagnitude(2) -- Масштаб искр
			effectdata:SetScale(1)
			effectdata:SetRadius(3)
			util.Effect("MetalSpark", effectdata, true, true)
			ply:EmitSound("physics/metal/metal_solid_impact_bullet"..math.random(4)..".wav", 80, 100)
		elseif GetRoleTableSH(org.owner:GetRoleName())["headgear"] then
			dmg = dmg / 32
			local ply = org.owner
			local hitpos = (hit and isvector(hit)) and hit or dmgInfo:GetDamagePosition()
			local effectdata = EffectData()
	...
	if org.owner:GTeam() == TEAM_SCP then
		dmg = dmg / 32
	end
	print(dmg)
	--	print(old_i_dmg)
...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	-- =======================================================
	-- ОТКЛЮЧЕНИЕ СИСТЕМЫ ОРГАНИЗМА ДЛЯ SCP-049
	-- =======================================================
	local isRobot = IsValid(ply) and ply.GTeam and ply:GTeam() == TEAM_AR
	local mdl = IsValid(ply) and ply:GetModel() or (IsValid(ent) and ent:GetModel())
	local isSCP = (mdl == "models/cultist/scp/scp_049.mdl" or mdl == "models/cultist/scp/scp_542.mdl")

	if isRobot or isSCP then
		...
		if isRobot then
			...
			effectdata:SetMagnitude(2) effectdata:SetScale(1) effectdata:SetRadius(2)
			util.Effect("Sparks", effectdata, true, true)
			ent:EmitSound("physics/metal/metal_solid_impact_bullet"..math.random(1,4)..".wav", 75, math.random(90, 110))
		end

		local dmg = dmgInfo:GetDamage()

		-- Толкаем физику, если они упали (в виде рэгдолла)
		if ent:IsRagdoll() then
			local phys = ent:GetPhysicsObjectNum(0)
			if IsValid(phys) then
				phys:ApplyForceOffset(dmgInfo:GetDamageForce() * 0.5, dmgInfo:GetDamagePosition())
			end
		end
		
		-- НАНОСИМ УРОН ПО HP НАПРЯМУЮ
		-- (Homigrad блокирует стандартный урон от пуль, поэтому делаем это вручную)
		if IsValid(ply) then
			-- Учитываем ScaleDamage (полезно для зомби или бронированных SCP)
			if ply.ScaleDamage then
				...
				local hitgroup = hg.bonetohitgroup and hg.bonetohitgroup[bonename] or HITGROUP_GENERIC
				
				local groupName = "HITGROUP_GENERIC"
				if hitgroup == HITGROUP_HEAD then groupName = "HITGROUP_HEAD"
				elseif hitgroup == HITGROUP_CHEST then groupName = "HITGROUP_CHEST"
				elseif hitgroup == HITGROUP_STOMACH then groupName = "HITGROUP_STOMACH"
				elseif hitgroup == HITGROUP_LEFTARM then groupName = "HITGROUP_LEFTARM"
				elseif hitgroup == HITGROUP_RIGHTARM then groupName = "HITGROUP_RIGHTARM"
				elseif hitgroup == HITGROUP_LEFTLEG then groupName = "HITGROUP_LEFTLEG"
				elseif hitgroup == HITGROUP_RIGHTLEG then groupName = "HITGROUP_RIGHTLEG"
				end
				
				dmg = dmg * (ply.ScaleDamage[groupName] or 1)
			end
		...
		-- Возвращаем true, чтобы полностью остановить дальнейшую (ванильную) логику урона
		return true
	end

	-- =======================================================
	-- СИСТЕМА GOC (80% шанс рикошета от пуль)
	-- =======================================================
	if IsValid(ply) and ply.GTeam and ply:GTeam() == TEAM_GOC then
		if dmgInfo:IsDamageType(DMG_BULLET) or dmgInfo:IsDamageType(DMG_BUCKSHOT) then
			-- Бросаем кубик (от 1 до 100). Если выпало 80 или меньше — рикошет!
			if math.random(1, 100) <= 80 then
				
				local hitpos = dmgInfo:GetDamagePosition()
				local dir = dmgInfo:GetDamageForce():GetNormalized()

				-- Спавним красивые искры
				local effectdata = EffectData()
				effectdata:SetOrigin(hitpos)
				...
				-- Звук отскока от тяжелой брони
				ent:EmitSound("physics/metal/metal_solid_impact_bullet"..math.random(1,4)..".wav", 80, math.random(95, 105))
				
				-- Эффект пролетающей мимо срикошетившей пули (если в вашей сборке есть этот звук)
				local rnd = math.random(1, 12)
				if rnd == 8 then rnd = 9 end
				ent:EmitSound("arc9_eft_shared/ricochet/ricochet" .. rnd .. ".ogg", 85, math.random(90, 110))

				-- Если GOC лежит в виде рэгдолла, даем легкий физический толчок (кинетическая энергия пули),
				-- но без крови, боли и потери ХП.
				if ent:IsRagdoll() then
					local tr = hg.GetTraceDamage(ent, hitpos, dmgInfo:GetDamageForce())
					local bone = tr and tr.PhysicsBone or 0
					local phys = ent:GetPhysicsObjectNum(bone)
					
					if IsValid(phys) then
						phys:ApplyForceOffset(dmgInfo:GetDamageForce() * 0.1, hitpos)
					end
				end

				-- Полностью отменяем урон, кровотечение и шок!
				return true
			end
		end
	end
	...
	-- EFFECT
-- EFFECT: Кровь и хэдшоты
	if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT + DMG_SLASH) then
		if dmgBlood > 1 and #inputHole > 0 then
			-- Вызываем стандартную систему крови из Homigrad (лужи/капли)
			net.Start("hg_bloodimpact")
			net.WriteVector(dmgPos)
			net.WriteVector(dirCool / 5) -- Увеличил разброс крови
			net.WriteFloat(dmg / 5)
			net.WriteInt(1, 8)
			net.Broadcast()

			-- Добавляем пышные эффекты крови Source Engine (как в phys_bullet)
			local effdata = EffectData()
			effdata:SetOrigin(dmgPos)
			effdata:SetNormal(-dirCool) -- Брызги летят навстречу пуле (выходное отверстие обрабатывается ниже)
			effdata:SetColor(0) -- 0 = красная кровь
			effdata:SetFlags(3) -- Флаги разброса
			--print(hitgroup)
			if (hitgroup == HITGROUP_HEAD) then
				--print("мяу")
				-- Если хэдшот: огромный всплеск
				--effdata:SetScale(3) 
				--effdata:SetMagnitude(2)
				--util.Effect("BloodImpact", effdata)
				--util.Effect("bloodspray", effdata) -- Дополнительный спрей
				--if ply.HeadResist > 0 or ply:GetRoleName():find("Juggernaut") then
				--ply:EmitSound("physics/metal/metal_solid_impact_bullet"..math.random(4)..".wav", 80, 100)
				
				-- Добавляем искры как в phys_bullet
				--local effectdata = EffectData()
				--effectdata:SetOrigin(dmgPos)
				--effectdata:SetNormal(-dirCool)
				--effectdata:SetMagnitude(2) -- Масштаб искр
				--effectdata:SetScale(1)
				--effectdata:SetRadius(3)
				--util.Effect("MetalSpark", effectdata, true, true)
				...
				-- Если тело: средний всплеск
				if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) then
					effdata:SetScale(1.5)
...
function ApplyPain(ply, number)
	ply.organism.painadd = ply.organism.painadd + number
end
