--\\ CL Bones
	if CLIENT then
		...
		local function ApplyHandPose(ent, hold_table, hand_mat, copyent)
			-- ОПТИМИЗАЦИЯ: не рендерим пальцы для тех, кто дальше 350 юнитов (их все равно не видно)
			if ent ~= LocalPlayer() and ent:GetPos():DistToSqr(EyePos()) > 122500 then return end

			local sorted = {}
			for name, invmat in pairs(hold_table) do
				table.insert(sorted, {name = name, invmat = invmat})
			end
			table.sort(sorted, function(a, b) return a.name < b.name end)

			for _, data in ipairs(sorted) do
				local name = data.name
				local invmat = data.invmat
				local bone = ent:LookupBone(name)
				
				if not bone or bone == -1 then continue end
				
				local current_mat = ent:GetBoneMatrix(bone)
				if not current_mat then continue end

				local target_mat
				if IsValid(copyent) and copyent:LookupBone(name) then
					local cp_mat = copyent:GetBoneMatrix(copyent:LookupBone(name))
					if cp_mat then target_mat = cp_mat end
				end
				
				if not target_mat then
					target_mat = hand_mat * invmat
				end

				target_mat:SetTranslation(current_mat:GetTranslation())

				-- КРИТИЧЕСКИЙ ФИКС: Используем SetBoneMatrix напрямую!
				-- hg.bone_apply_matrix рекурсивно двигает дочерние кости. Из-за этого старый код
				-- смещал фаланги пальцев по 3-4 раза подряд, убивая процессор.
				ent:SetBoneMatrix(bone, target_mat)
			end
		end
		...
		function hg.set_hold(ent, hold, copyent)
			...
			ent.hold = hold
			if hand_poses[hold] then
				ApplyHandPose(ent, hand_poses[hold], lhmat, copyent)
			end
		end

		function hg.set_holdrh(ent, hold, copyent)
			local lh = ent:LookupBone("ValveBiped.Bip01_R_Hand")
			if not lh then return end
			local lhmat = ent:GetBoneMatrix(lh)
			ent.holdrh = hold
			if hand_posesrh[hold] then
				ApplyHandPose(ent, hand_posesrh[hold], lhmat, copyent)
			end
		end
		...
		function hg.ShouldTPIK(ply, wpn)
			...
			if ply == lply or ply == lply:GetNWEntity("spect") then return true end
			
			local time = CurTime()
			if (ply.cachedtpik or 0) > time then return ply.cachedval end
			ply.cachedtpik = time + 0.2 -- Кэшируем результат, чтобы не считать каждый кадр

			-- ОПТИМИЗАЦИЯ: TPIK для других игроков работает только в радиусе ~300 юнитов 
			local distSqr = ply:GetPos():DistToSqr(EyePos())
			if distSqr > 90000 then 
				ply.cachedval = false 
				return false 
			end

			-- ОПТИМИЗАЦИЯ: Если игрок за нашей спиной или вне поля зрения, отключаем ему TPIK
			local dir = ply:GetPos() - EyePos()
			dir:Normalize()
			if EyeAngles():Forward():Dot(dir) < 0.2 then
				ply.cachedval = false
				return false
			end
