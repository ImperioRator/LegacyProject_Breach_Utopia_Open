...
function hg.cam_things(ply, view, angles)
	...
	local vela = (ply:GetMoveType() == MOVETYPE_NOCLIP or ply:GetMoveType() == MOVETYPE_OBSERVER or (ply.GTeam and ply:GTeam() == TEAM_SPEC)) and Vector(0,0,0) or -(hg.GetCurrentCharacter(ply):GetVelocity() / 50)
...
local IsValid = IsValid
local isRendering = false -- ВАЖНО: Флаг защиты от зацикливания

--local function renderscene(pos, angle, fov)
	--if isRendering then return end -- Прерываем бесконечный цикл скайбокса
--
...
--
	--isRendering = true -- Включаем блокировку хука перед рендером
	--render_RenderView(renderView)
	--isRendering = false -- Снимаем блокировку хука после рендера
--
	--lply.norender = nil
	--
	--if invert then
...
hook.Remove("Bones","HeadTurnAway",function(ply)
