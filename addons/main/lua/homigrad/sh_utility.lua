...
function hgIsDoor(ent)
	if not ent or not IsValid(ent) or ent == NULL then return false end
...
--\\ custom spawn
	...
	hook.Add("player_spawn", "homigrad-spawn3", function(data)
		...
		timer.Simple(0, function()
			...
			if ply:GetNetVar("handcuffed") == true then
				ply:SetWalkSpeed(100)
				ply:SetRunSpeed(100)
			end
...
--\\ DrawPlayerRagdoll
	...
	local angfuck = Angle()
	function DrawPlayerRagdoll(ent, ply, flags)
        ...
        if not ent.GetManipulateBoneScale or not lkp then return end

        if hg.RenderWeapons then hg.RenderWeapons(ent, ply) end

        -- =========================================================
        -- ИСПРАВЛЕННЫЙ ФИКС КАДРА (Через системные флаги GMod)
        -- =========================================================
        local currentFrame = FrameNumber()
        -- 1073741824 - это STUDIO_SHADOWDEPTHTEXTURE (флаг рендера теней в GMod)
        local isShadow = flags and (bit.band(flags, 1073741824) != 0) or false

        if isShadow or ent.LastDrawPlayerRagdollFrame ~= currentFrame then
            -- Если мы рисуем тени, мы НЕ блокируем кадр, чтобы основная отрисовка обновила свет оружия
            if not isShadow then
                ent.LastDrawPlayerRagdollFrame = currentFrame
            end
            ...
            if IsValid(ply.OldRagdoll) and hg.SmoothUnfake then
                hg.SmoothUnfake(ent, ply)
            end

            if ply.GetNetVar and ply:GetNetVar("handcuffed", false) and hg.CuffedAnim then 
                hg.CuffedAnim(ent, ply) 
            end
        end
        -- =========================================================
        ...
        local armors = ply.GetNetVar and ply:GetNetVar("Armor") or ent.PredictedArmor
        local hideArmorRender = ply.GetNetVar and ply:GetNetVar("HideArmorRender", false) or ent.PredictedHideArmorRender
        if armors and next(armors) and not hideArmorRender and RenderArmors then
            RenderArmors(ply, armors, ent)
        end

        if hg.RenderBandages then hg.RenderBandages(ent, ply) end
        if hg.RenderTourniquets then hg.RenderTourniquets(ent, ply) end
        if hg.GoreCalc then hg.GoreCalc(ent, ply) end

        local fountains = GetNetVar("fountains") or {}
        local wawanted = (GetViewEntity() != ply) and not fountains[ent] and not (not lply:Alive() and lply:GetNWEntity("spect") == ply and viewmode == 1) and vector_full or vector_small
        
        local mat = ent:GetBoneMatrix(lkp)
        if mat then
            if hg_gopro and not hg_gopro:GetBool() then
                mat:SetScale(wawanted)
            end
            if hg.bone_apply_matrix then hg.bone_apply_matrix(ent, lkp, mat) end
        end

        if hg.ProjectilesDraw then hg.ProjectilesDraw(ent, ply) end
        if ply.GetNetVar and ply:GetNetVar("headcrab") and hg.RenderHeadcrab then hg.RenderHeadcrab(ent, ply) end
    end
--//
--\\ Is Local
...
--\\ Render Override
	hg.renderOverride = function(self, ent, flags)
		...
		DrawPlayerRagdoll(ent, self,flags)
		...
		if ent:IsPlayer() and IsValid(ent:GetActiveWeapon()) and ent:GetActiveWeapon():GetClass() == "item_scp_268" and ent:GetActiveWeapon():GetNWInt("HatState", 0) == 2 then return end
		--if ent:GetModel() == "models/1000shells/scp966/scp_966.mdl" and LocalPlayer().NVG != true then return end
		--print(LocalPlayer())
		ent:DrawModel()
...
--\\ Custom Hull check
	...
	function hg.hullCheck(startpos, endpos, ply)
	    -- ДОБАВЛЕНА ПРОВЕРКА: Если игрок в технике, наблюдатель или в ноуклипе - камера не сталкивается со стенами
	    if ply:InVehicle() or (ply.GTeam and ply:GTeam() == TEAM_SPEC) or ply:GetMoveType() == MOVETYPE_NOCLIP then 
...
--\\ Custom ents trace functions
	...
	function hg.eye(ply, dist, ent, aimvec, startpos)
		...
		local pos = startpos or headm:GetTranslation() + (fakeCam and (headm:GetAngles():Forward() * 2 + headm:GetAngles():Up() * -2 + headm:GetAngles():Right() * 3) or (eyeAng:Up() * 5 + headm:GetAngles():Right() * 4 + headm:GetAngles():Up() * 0  + headm:GetAngles():Forward() * (4 + (ply.PlayerClassName == "Combine" and 4 or 0))))
...
--\\ Suicide
	if SERVER then
		concommand.Add("suicide", function(ply)
			--if IsGov(ply:GetPlayerClass()) then return end
...
--\\ Custom movement
	...
	hook.Add("SetupMove", "HG(StartCommand)", function(ply, mv, cmd)
		...
		if (ply:GetMoveType() == MOVETYPE_NOCLIP or ply:GetMoveType() == MOVETYPE_OBSERVER or (ply.GTeam and ply:GTeam() == TEAM_SPEC)) then
		...
		local isSCP = ply.GTeam and ply:GTeam() == TEAM_SCP
		if isSCP then
			cmd:RemoveKey(IN_SPEED) -- Убираем кнопку шифта для движка (чтобы не было анимации бега)
		end

		local runnin = ply:KeyDown(IN_SPEED) and not ply:Crouching() and not isSCP
		...
		if isSCP then
			ply.CurrentSpeed = walk_speed
		elseif(runnin and velLen >= 10)then
		...
		local move = ply:GetRunSpeed() * 0.85
		if ply.GTeam and ply:GTeam() == TEAM_AR then
			move = 250
		end
	--k = 1 * weightmul
		k = 1
		k = k * math.Clamp(consmul, 0.9, 1) -- Был 0.7
		k = k * math.Clamp((org.stamina and org.stamina[1] or 180) / 120, 0.3, 1) -- Был 0.3
		k = k * math.Clamp(5 / ((org.immobilization or 0) + 1), 0.7, 1) -- Был 0.25
		k = k * math.Clamp((org.blood or 0) / 5000, 0.6, 1) -- Был 0
		k = k * math.Clamp(10 / ((org.shock or 0) + 1), 0.8, 1) -- Был 0.25
		--k = k * math.Clamp((org.lleg and org.lleg >= 0.5 and math.max(1 - org.lleg, 0.6) or 1) * (org.lleg and org.rleg >= 0.5 and math.max(1 - org.rleg, 0.6) or 1) * ((org.analgesia * 1 + 1)), 0.6, 1) -- Был 0
		--k = k * (org.llegdislocation and 0.9 or 1) * (org.rlegdislocation and 0.9 or 1) -- Был 0.75
		--k = k * (org.pelvis == 1 and 0.8 or 1) -- Был 0.4
		...
		k = k * math.Clamp(20 / ((org.pain or 0) + 1), 0.6, 1) -- Был 0.01 (ОЧЕНЬ сильное замедление от боли)

		local slwdwn = ply:GetNetVar("slowDown", 0)
		if(slwdwn > 0)then
			k = k * math.Clamp((250 - slwdwn) / 250, 0.85, 1) -- Был 0.75
		end

		k = math.max(k, 0.45) -- Минимальный предел скорости теперь 45%, а не 10% (20 / 200)
...
--\\ custom footsteps
	...
	hook.Add("PlayerFootstep", "CustomFootstep2sad", function(ply, pos, foot, sound, volume, rf)
		...
		if SERVER then
			...
			if ply:GetModel() == "models/cultist/scp/scp_049.mdl" then
				ply:EmitSound("nextoren/scp/049/step"..math.random(1, 3)..".ogg", 75, math.random(95, 105), 100)
				return true
			end

			if ply:GetModel() == "models/1000shells/scp966/scp_966.mdl" then
				--ply:EmitSound("nextoren/scp/049/step"..math.random(1, 3)..".ogg", 75, math.random(95, 105), 100)
				return true
			end

			if ply.GTeam and ply:GTeam() == TEAM_AR then
				ply:EmitSound("^nextoren/charactersounds/foley_goc/gear"..math.random(1, 6)..".wav", 75, math.random(90, 110), volume * 0.8)
				return true
			end
			...
			if not (ply:IsWalking() or ply:Crouching()) then
				...
				-- SoundDuration на сервере для аддонов может возвращать 0. 
				if SoundDuration(snd) <= 0 or ply.PlayerClassName == "Gordon" then
					snd = sound
				end
				
				local pitch = changePitch(math.random(95, 105))
				local sound_ent = IsValid(ent) and ent or ply
				
				-- Вызываем метод энтити, он работает 100% стабильно по PVS
				--sound_ent:EmitSound(snd, 75, pitch, volume, CHAN_AUTO)
				ply:EmitSound(snd, 75, math.random(90, 110), volume * 0.8)
				return true
			end
		end
...
--\\ Can use hands
	function hg.CanUseLeftHand(ply)
	    ...
	    local deploying = wep and (
	        wep.deploy and (wep.deploy - CurTime()) > ((wep.CooldownDeploy or 1) / 2) 
	        or wep.holster and (wep.holster > CurTime()) and (wep.holster - CurTime()) < ((wep.CooldownHolster or 1) / 2)
	    )
	    ...
	    return (not ((((ply:GetTable().ChatGestureWeight or 0) > 0.1 or
	        ...
	        IsValid(ply.flashlight)) and not ply:GetNetVar("handcuffed") and (wep and not wep.reload)) or
	        (deploying) or
	        (ent ~= ply and math.abs(ent:GetManipulateBoneAngles(ent:LookupBone("ValveBiped.Bip01_L_Finger11"))[2]) > 5 and not ply:InVehicle()) or
...
if SERVER then
util.AddNetworkString("HG_ShakeScreen")

    net.Receive("HG_ShakeScreen", function()
        local amp = net.ReadFloat()
        local freq = net.ReadFloat()
        local dur = net.ReadFloat()

        local ang = Angle(math.random(-amp, amp), math.random(-amp, amp), 0)
