...
hook.Remove("Player Think", "homigrad-dropholstered", function(ply)
...
--ook.Add("PlayerDroppedWeapon", "ManualPickup", function(owner, wep)
--	wep.IsSpawned = true
--nd)
--
--ook.Add("PlayerSpawnedSWEP", "ManualPickup", function(ply, wep) wep.IsSpawned = true end)
...
--			ply:DropDRPWeapon(wep, nil, not IsValid(wep.fakeGun) and (eyeAngles:Forward() * (isnumber(vel) and vel or 250)) + ply:GetVelocity() or nil)
--		end
--	end
...
hook.Add( "Move", "hg_RagdollIntoWalls", function( ply, mv)
	if ply.GTeam and ply:GTeam() == TEAM_SPEC then return end
...
hook.Remove("PostEntityTakeDamage", "GlassShards", function(ent, dmginfo)
...
-- hook.Remove("Org Think", "ColdMaps", function(owner, org, timeValue)
...
local lastThinkPlayer = 0
hook.Add("PlayerTick", "ilovefurries", function(ply)
	-- if lastThinkPlayer > CurTime() then return end
	-- lastThinkPlayer = lastThinkPlayer + 0.1
...
hook.Add("Player Think", "homigrad-viewoffset", function(ply)
	if lastThink and lastThink > CurTime() then return end
...
-- [ЗАЩИТА ЛИЦА / ЛОГИЧЕСКИЙ ШЛЕМ]
hook.Add("PreTraceOrganBulletDamage", "Helmet_AntiOneshot", function(org, bone, dmg, dmgInfo, box, dir, hit, ricochet, organ, hook_info)
	local ply = org.owner
	if not IsValid(ply) then return end

	local organName = organ[1]

	-- Если пуля попадает в мозг, череп или челюсть
	if organName == "brain" or organName == "skull" or organName == "jaw" then
		-- Проверяем, есть ли на голове игрока шлем
		local armors = ply.armors or ply:GetNetVar("Armor", {})
		local headArmor = armors["head"]
		
		if headArmor then
			local helmData = hg.armor["head"][headArmor]
			-- Если это полноценный шлем (защита >= 4)
			if helmData and (helmData.protection or 0) >= 4 then
				-- 1. Полностью отменяем урон по мозгу/черепу (Игрок не умрет мгновенно)
				hook_info.restricted = true
				hook_info.dmg = 0
				
				-- 2. Срезаем урон самой пули в 10 раз и убираем кровотечение (DMG_CLUB)
				dmgInfo:ScaleDamage(0.1)
				dmgInfo:SetDamageType(DMG_CLUB)
				
				-- 3. Эффект контузии от попадания в каску
				ply:EmitSound("physics/metal/metal_solid_impact_bullet"..math.random(4)..".wav", 80, 100)
				if ply:IsPlayer() then
					ply:ViewPunch(AngleRand(-5, 5))
					if ply.AddTinnitus then ply:AddTinnitus(2, true) end
				end
			end
		end
	end
...
-- Полное удаление 3D-скайбокса для предотвращения краша CUtlLinkedList
hook.Add("InitPostEntity", "Disable3DSkybox_CrashFix", function()
