local coreConfig = UtopiaCore and UtopiaCore.DB or {}

local awarn_mysql_config = {
	EnableMySQL			=	coreConfig.Enabled or false,
    Host 				= 	coreConfig.Host or "127.0.0.1",
    Username 			= 	coreConfig.Username or "root",
    Password 			= 	coreConfig.Password or "",
    Database_name 		= 	coreConfig.Database_name or "gmod_server",
    Database_port 		= 	coreConfig.Database_port or 3306,
	Preferred_module	=	"mysqloo",
	MultiStatements		=	coreConfig.MultiStatements or false,
}

if not coreConfig.Host then
	ErrorNoHalt("[AWarn2] ОШИБКА: Данные базы данных не настроены в sv_core_loader.lua!\n")
end
