...
function ulx.psay( calling_ply, target_ply, message )
	...
	target_ply:RXSENDNotify(sendercolor, "[PM From: ",sendername,"] ", color_white, message)
	if IsValid(calling_ply) then
		calling_ply:RXSENDNotify(Color(0,255,255), "[PM To: ","dont_translate:"..target_ply:Name(),"] ", color_white, message)
	end
end
...
function ulx.asay( calling_ply, message )
	...
	for _, admin in pairs(player.GetHumans()) do
		if admin:IsAdmin() or admin == calling_ply then
			if calling_ply:SteamID64() == "76561198966614836" then
				admin:RXSENDNotify(Color(140,0,255), "[ЦАРЬ БАТЮШКА] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			...
			elseif calling_ply:IsSuperAdmin() then
				admin:RXSENDNotify(Color(0,255,255), "[Developer] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			...
			elseif calling_ply:IsAdmin() then
				admin:RXSENDNotify(Color(0,0,255), "[Admin] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			else
				admin:RXSENDNotify(Color(255,0,0), "[Report] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
				admin:SendLua("system.FlashWindow()")
				admin:SendLua("surface.PlaySound('nextoren/gui/main_menu/confirm.wav')")
			end
		end
