	...
	for i, weapon in pairs (LCWeps) do
		if(weapon!="LC_Already_Taken") then
			...
			WepBut.Paint = function( self, w, h )
				...
				local texture = weapons.GetStored( weapon ).InvIcon || Material("nextoren/gui/new_icons/missing.png")
...
local function DrawWepBox()
	...
	function LC_WeaponIcon:Paint( w, h )
        ...
        local texture = weapons.GetStored( LCWeps[HoveredID] ).InvIcon || Material("nextoren/gui/new_icons/missing.png")
...
hook.Add( "KeyPress", "KeyPressForRagdoll", function( ply, key )
	...
	if ( key == IN_USE ) then
		...
		--ply:BrProgressBar( "Обыскивание", 6, "nextoren/gui/new_icons/notifications/breachiconfortips.png" )
...
net.Receive("OpenLootMenu", function( len )
    ...
    OpenLootMenu(ply, ent, vtable.Items, vtable.Name)
end)
